...
*/
/**
 * Shikola Teacher Portal - Offline Manager
...
(function() {
    ...
    const OFFLINE_CONFIG = {
        cachePrefix: 'shikola_teacher_',
        cacheExpiry: {
            dashboard: 5 * 60 * 1000,        // 5 minutes
            timetable: 15 * 60 * 1000,      // 15 minutes
            classes: 20 * 60 * 1000,         // 20 minutes
            attendance: 2 * 60 * 1000,       // 2 minutes
            assignments: 10 * 60 * 1000,      // 10 minutes
            grades: 30 * 60 * 1000,           // 30 minutes
            pupils: 15 * 60 * 1000,          // 15 minutes
            notifications: 5 * 60 * 1000       // 5 minutes
        },
        maxStorageSize: 30 * 1024 * 1024, // 30MB
        syncRetryAttempts: 3,
        syncRetryDelay: 2000
    };
    ...
    class RealtimeManager {
        ...
        static handleRealtimeUpdate(type, data) {
            ...
            if (window.teacherRealtimeUpdate) {
                window.teacherRealtimeUpdate(type, data);
            }
    ...
    function updateConnectionStatus() {
        ...
        if (wasOnline !== isOnline) {
            ...
            if (window.teacherConnectionStatusChanged) {
                window.teacherConnectionStatusChanged(isOnline);
            }
        }
    }
    ...
    async function syncPendingActions() {
        ...
        for (const action of pendingSyncActions) {
            try {
                await TeacherAPI.request(action.endpoint, action.options);
    ...
    function addPendingAction(endpoint, options) {
        ...
        if (pendingSyncActions.length > 30) {
            pendingSyncActions = pendingSyncActions.slice(-30);
        }
    }

    // Public API
    window.TeacherOfflineManager = {
        // API methods
        request: TeacherAPI.request.bind(TeacherAPI),
        syncData: TeacherAPI.syncData.bind(TeacherAPI),
        ...
        init: function() {
            ...
            console.log('Teacher Offline Manager initialized');
        }
    };

    // Auto-initialize when DOM is ready
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => window.TeacherOfflineManager.init());
    } else {
